...
def get_scratch_model() -> keras.Model:
    ...
    if _scratch_model is None:
        ...
        _scratch_model = keras.models.load_model(str(SCRATCH_MODEL))
...
def get_tl_model() -> keras.Model:
    ...
    if _tl_model is None:
        ...
        _tl_model = keras.models.load_model(str(TL_MODEL))
    return _tl_model

def preprocess_for_scratch(pil_img: Image.Image) -> np.ndarray:

    img = pil_img.resize(SCRATCH_IMG_SIZE, Image.LANCZOS)
    arr = np.array(img, dtype=np.float32) / 255.0
    return np.expand_dims(arr, axis=0)        # (1, 150, 150, 3)


def preprocess_for_tl(pil_img: Image.Image) -> np.ndarray:

    img = pil_img.resize(TL_IMG_SIZE, Image.LANCZOS)
...
def predict():
    ...
    try:
        ...
        if SCRATCH_MODEL.exists():
            ...
            probs = get_scratch_model().predict(arr, verbose=0)[0]
        ...
        if TL_MODEL.exists():
            ...
            probs = get_tl_model().predict(arr, verbose=0)[0]
